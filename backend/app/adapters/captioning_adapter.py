"""
Scene Captioning & Land-Cover Description Model Adapter.

Integrates the BigEarthNet-19 adapted neural classifier for multi-spectral land-cover
taxonomy mapping and descriptive natural-language remote sensing scene synthesis.
"""

import os
import logging
from typing import List, Dict, Any, Tuple, Optional
from app.adapters.base import BaseModelAdapter
from app.db.models import ImageAsset
from app.demo.mock_engines import DemoExecutionEngine
from app.models.manager import model_manager
from app.models.bigearthnet_model import load_bigearthnet_model, classify_land_cover

logger = logging.getLogger(__name__)

class CaptioningAdapter(BaseModelAdapter):
    def __init__(
        self, 
        model_id: str = "rs-captioning-landcover", 
        name: str = "BigEarthNet Land-Cover & Captioning Specialist", 
        version: str = "1.0.0",
        backend: str = "adapted"
    ):
        super().__init__(model_id=model_id, name=name, version=version, task="captioning", backend=backend)

        # Register factory with ModelManager
        if self.backend == "adapted":
            model_manager.register_factory(
                model_id=self.model_id,
                factory=lambda: load_bigearthnet_model(device=model_manager.device),
                backend_type="adapted",
                metadata={"architecture": "MobileNetV3-Small (Adapted)", "taxonomy": "BigEarthNet-19"}
            )

    def validate_input(self, assets: List[ImageAsset]) -> Tuple[bool, List[str]]:
        errors = []
        if len(assets) != 1:
            errors.append(f"Captioning specialist requires exactly 1 satellite image. Received {len(assets)}.")
        return len(errors) == 0, errors

    def _resolve_asset_path(self, asset: ImageAsset) -> Optional[str]:
        """Prefers preprocessed RGB preview image; falls back to original raster file path."""
        if asset.preview_path and os.path.exists(asset.preview_path):
            return asset.preview_path
        if asset.file_path and os.path.exists(asset.file_path):
            return asset.file_path
        return None

    def analyze(
        self, 
        query: str, 
        assets: List[ImageAsset], 
        context: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        valid, errors = self.validate_input(assets)
        if not valid:
            raise ValueError(f"Input validation failed for Captioning: {', '.join(errors)}")

        asset = assets[0]

        # 1. Fallback to Demo Engine if explicitly configured
        if self.backend == "demo":
            return DemoExecutionEngine.execute_captioning(query, assets)

        # 2. Check if physical image exists on disk
        image_path = self._resolve_asset_path(asset)
        if not image_path:
            logger.info(f"Physical image file not on disk for asset '{asset.id}'. Returning demo execution.")
            return DemoExecutionEngine.execute_captioning(query, assets)

        # 3. Adapted AI Inference via BigEarthNet-19 model
        try:
            model = model_manager.get_model(self.model_id)
            if model is None:
                logger.warning(f"Could not load '{self.model_id}'. Falling back to demo mode.")
                res = DemoExecutionEngine.execute_captioning(query, assets)
                res["metadata"]["fallback_reason"] = "Model weights could not be loaded."
                return res

            classification = classify_land_cover(
                model=model,
                image_path=image_path,
                device=model_manager.device,
                top_k=4
            )

            caption = classification["caption"]
            confidence = classification["confidence"]

            evidence_item = {
                "type": "scene_caption",
                "description": caption,
                "primary_class": classification["primary_class"],
                "top_classes": classification["top_classes"],
                "imageUrl": asset.preview_url or f"/static/uploads/{asset.filename}",
                "scaleLabel": f"GSD: {asset.resolution or '10m'}",
                "satelliteLabel": f"{asset.modality.upper() if asset.modality else 'Optical'} · {asset.filename}",
                "confidence": confidence,
                "is_demo": False
            }

            return {
                "task": "captioning",
                "backend": "adapted",
                "confidence": confidence,
                "answer": caption,
                "evidence": [evidence_item],
                "metadata": {
                    "model_id": self.model_id,
                    "model_name": classification["model_name"],
                    "taxonomy": classification["taxonomy"],
                    "top_classes": classification["top_classes"],
                    "confidence_source": "BigEarthNet sigmoid class activation",
                    "is_demo": False
                }
            }

        except Exception as e:
            logger.error(f"Error during BigEarthNet captioning inference: {e}", exc_info=True)
            raise RuntimeError(f"Captioning execution failed: {e}")
